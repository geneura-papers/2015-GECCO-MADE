...
\title{\ \\ \LARGE\bf Massive emergence of monomyths in non-player
  characters} \notajj{Quito el genitivo y además open worlds
...
drastically by it. \notajj{This is getting really old. What is the
  point of this particular paper}
...
\textit{the monomyth}, a complex behavior pattern that links different
characters and archetypes and which is also called \textit{the hero's journey}, has
to emerge. We formally analyze the archetypes present in \textit{the
  monomyth} using predicate logic and provide the basis to model facts
...
automatons, and fitness functions using logical deductions on the
world's facts, in a hybrid three-layered \textit{Evolutionary
  Computation - Agent Based Model} approach that optimally
parametrizes the agents. 

The analysis of the results show that we are able to obtain the
parameters of characters that, after the execution in a massive
multi-agent virtual simulation, better allow the emergence of the
complex patterns present in \textit{the monomyth}. The agents
\notajj{Characters or agents?}
 generated in each execution can be used as characters that populate
 an open world in a videogame, providing backstories that the user may
 perceive as interesting and unique. \notajj{May or may not? You have
   to do strong claims in the abstract. Will you prove that or not?
   What makes something interesting or unique?} 
This method can also be applied to different game settings or
characters' nature by re-implementing the Agent's layer using the
rules provided in this paper.\notaruben{Quiza me enrollo mucho en las
  implicaciones, aunque creo que resume los objetivos desde el punto
  de vista de aplicacion. Por supuesto, tambien hablo de los
  resultados, asi que estaría bien revisarlo cuando los tengamos para
  ver si es verdad} \notajj{Efectivamente, esto no pertenece al abstract}

\end{abstract}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\notaruben{Titulo provisional... muchos genitivos sajones... Que alguien creativo haga un titulo impactante :-)}
\notajj{Quitados genitivos por lo pronto. {\em Open world} tiene un
  significado particular, que no sé si habría que poner aquí}
