...
backstories was the lack of formal models to define the archetypes or the events in the virtual world, hence no {\em a priori} rules
could be provided to establish whether or not a design of an agent
could lead to the appearance of the archetypes. For example, if the agent has no actions that fire events related to 'encouraging', no archetype 'herald' could ever emerge, as we discuss on section~\ref{sec:met}. For this reason, in the present paper, we formally model the archetypes using predicate logic. 
