...
% Antonio - yo quitarí esta frase, o pondría más citas, una de dos. ;) FERGU: añadida referencia
...
% Vale, lo pongo primero en español y luego intento hacerlo en inglés. En este criterio de parada hemos empleado el concepto estadístico de frontera siperior para definir un umbral. Por definición, todo valor de la muestra que supere esa frontera está considerado un punto outsider. Para calcular dicha frontera se necesitan los cuartiles de la muestra Q1 y Q3, con los que se calcula el rango intercuartílico (IQR). El valor de la frontera superior se calcula como = Q3 + 1.5 + IQR . Sin embargo, el valor 1.5 puede ser ajustado a voluntad se denomina rango de la frontera. En nuestro caso, hemos empleado el valor estándar de 1.5 así como valores menos restrictivos (1.0) y más restrictivos (2.0 y 2.5).
...
% Antares - @Antonio eso era antes, la idea es que N es siempre el tamaño de la población, y en este caso, n el valor máxmo de reemplazamiento.
% Antonio - @Antares, cuidao con la variable 'N', que es el número de batallas en la función Score... ahora está bien.
...
% Antares - Ya el problema ha sido que los valores ``chulos'' no arrojasen resultados. De todos modos, es lo que te comenté una distribución uniforme desde la ``mitad'' del fitness hasta el valor máximo.
...
% Antares - Sí se contempla el ruido, ya que se siguen haciendo re-evaluaciones. 
...
Four possible values will be tested: 3, 7, 10 and 15 generations. %maribel y por qué estos? está fijado antes en algún otro paper como el del número de gneraciones?
...
% Antonio - Claro, el fitness cambia, pero se considera el fitness mejor global en cada generación, no el del individuo que se considerame el mejor. Lo he aclarado en el texto.
% Antares - si pero el ``fitness'' máximo de la población alcanzado hasta el momento no cambia, que es con el que se realiza la comparación.
\end{itemize}
...
% Antares - @Antonio: No sé que ha pasado que se han machacado los antiguos, seguramente sea culpa mía. Arrelado.
...
As it can be also seen in the figure all the criterion are well defined, since all the scores grow in every criterion block, so as more restrictive the criterion is, the higher the obtained score is. The score function is working well, even with the presence of noise, as the wide of the boxplots (score dispersion) demonstrates when the criteria become more restrictive (they are met with a lower probability). This is true in all the cases except in the \textit{Age of Outliers} which is so far the criterion with the worse results, as it will be proved in further analyses.
...
% Antares - @Antonio : He cambiado la gráfica, echa un ojo a ver que te parece

\begin{figure}[h!tb]
\begin{center}

\subfigure[]{\includegraphics[width=0.70\columnwidth]{imags/Comparative-log.eps} }
\end{center} 
...
% Antares: @Antonio, por seguir una nomenclatura común, pero si quieres podría intentar cambiarlo, pero me iba a llevar tiempo.
...
% Antares - ¿Se podría comentar también que alguno de los criterios de parada podría ser empleado para algún mecanismo diferente del algoritmo, como un
% ``cataclismo'' o una variación de los pesos o algo así? 

\begin{figure}[h!tb]
\begin{center}
\includegraphics[width=\columnwidth]{imags/victories.eps}
\end{center} 
\caption{Victories versus... TODO } %Antares - TODO: Ahora lo completo
\label{fig:vctories}
\end{figure}
...
\hline 
\multirow{2}{*}{{\tiny{SC }}} & {\tiny{AVERAGE}} & {\tiny{AVERAGE}} & {\tiny{COMPLETION}} &  & {\tiny{INDEX}} & {\tiny{INDEX}} & {\tiny{INDEX}}\tabularnewline
 & {\tiny{GENERATIONS }} & {\tiny{AVG. SCORE }} & {\tiny{RATE}} & {\tiny{VICTORIES (\%)}} & {\tiny{GENERATIONS }} & {\tiny{SCORE}} & {\tiny{VICTORIES}}\tabularnewline
{\tiny{FT\_22.0 }} & {\tiny{127.56 }} & {\tiny{22.65 }} & {\tiny{100\%}} & {\tiny{58.25 }} & {\tiny{4.25 }} & {\tiny{1.39 }} & {\tiny{1.27 }}\tabularnewline
{\tiny{FT\_24.0 }} & {\tiny{276.71 }} & {\tiny{24.39 }} & {\tiny{77.77\%}} & {\tiny{63.39 }} & {\tiny{9.22 }} & {\tiny{1.50 }} & {\tiny{1.38 }}\tabularnewline
{\tiny{FT\_26.0 }} & {\tiny{378.88 }} & {\tiny{26.45 }} & {\tiny{22.22\% }} & {\tiny{74.75 }} & {\tiny{12.63 }} & {\tiny{1.62 }} & {\tiny{1.63 }}\tabularnewline
\hline 
{\tiny{FI\_03.0 }} & {\tiny{10.31 }} & {\tiny{13.34 }} & {\tiny{100\%}} & {\tiny{30.00 }} & {\tiny{0.34 }} & {\tiny{0.82 }} & {\tiny{0.65 }}\tabularnewline
{\tiny{FI\_07.0 }} & {\tiny{24.56 }} & {\tiny{15.54 }} & {\tiny{100\%}} & {\tiny{41.39 }} & {\tiny{0.82 }} & {\tiny{0.95 }} & {\tiny{0.90 }}\tabularnewline
{\tiny{FI\_10.0 }} & {\tiny{35.47 }} & {\tiny{16.50 }} & {\tiny{100\%}} & {\tiny{47.94 }} & {\tiny{1.18 }} & {\tiny{1.01 }} & {\tiny{1.04 }}\tabularnewline
{\tiny{FI\_15.0 }} & {\tiny{52.22 }} & {\tiny{17.56 }} & {\tiny{100\%}} & {\tiny{53.00 }} & {\tiny{1.74 }} & {\tiny{1.08 }} & {\tiny{1.15 }}\tabularnewline
...
% Antares - Añado las conclusiones primero en español:
%El peor criterio de parada encontrado es el método \textit{[AO] Age of Outliers} basado en la edad de los individuos de la población, que es el método que consigue peores resultados en el score ofreciendo una convergencia prácticamente nula de las soluciones encontradas en la población al aumentar lo restrictivo del método. Esto puede ser un indicativo de que la antiguedad de los individuos no es representativo de la bondad de los individuos. Además, el test de Kruskal-Wallis, determinó que exite poca variación entre las muestras de las distintas variaciones del método.

%Por otro lado, el mejor criterio encontrado respecto a la bondad de los individuos es el \textit{[FT] Fitness Threshold} basado en umbrales de fitness. Sin embargo, las dificultades para definir un umbral de fitness correcto en un problema de nuestra naturaleza hacen este método no recomendado, aunque si ideal si puede determinarse. Además, es el método que más generaciones requiere, con el riesgo de que la ejecución del algoritmo no llegue a terminar, al estancarse la población y no ser capaz de convergir al valor frontera.

% El método basado en \textit{[FI] Fitness Improvement} es un método bastante rápido (en cuanto a generaciones), pero que no alcanza resultados muy buenos (en cuanto score). A pesar de que no pueda ser empleado como criterio de parada óptimo, su uso puede resultar interesante como apoyo al algoritmo como mecanismo de multiarranque del algoritmo o de cataclismo.

%Por último, el método de \textit{[FT] Fitness Threshold} es el que ofrece unos resultados más consistentes en todas las métricas. Dado que se basa en la tasa de reemplazo de los individuos, en cierta manera está midiendo el constante crecimiento del fitness dado que los individuos nuevos tienen que tener mayor fitness para entrar en el pull de la nueva población, pero sin hacer uso del propio fitness (que sabemos que por su naturaleza ruidosa no es totalmente confiable). 


%Antares - Blablabla correspondencia lineal entre los resultados de la batería de test de bondad y el score con un $R^2=0.9351$
