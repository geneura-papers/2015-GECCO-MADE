...
%           De todas formas esta frase no tiene mucho sentido: "las reglas aseguran que no haya una sola forma par crear el mejor bot; que si se encuentra, no hay más desafío"... ¿qué tiene que ver esto último con que no haya un óptimo conocido?
...
Other possibilities, such as certain level of victories (or fitness quality) over external enemies as a termination condition, present the problem that the bots will be overfitted to those particular enemies. 
...
extensively in the literature, but the conclusions to our study could, in principle, be extended to any RTS or even any game with non-deterministic match results and with an unknown best solution. 
...
to find the best autonomous  
agent for playing the game, i.e. the best \textit{bot}
\cite{Agent_Smith_CEC2009,bots:evostar,unreal:gecco10,DBLP:conf/cig/Fernandez-AresGMG12}
through an evolutionary process.
And the termination condition is a key factor in any algorithm and its experimental setup since it affects the algorithmic performance, with respect to the quality of the yielded solution, and also to the amount of resources devoted to the run. 
In EAs \cite{EAs_Back96} the usual termination condition is a constant number of generations (or evaluations), 
...
which the best solution is not improved (distance to the optimum is not reduced), among others \cite{RocheTermination13}. However, neither of them might be useful in certain problems such as games, mainly due to the noisy nature of the fitness function \cite{merelo14:noisy,bots:evostar}. 

This \textit{noise} is inherent to most games, since the enemy behavior, the
game environment, the bot itself, or all of them, may have an
...
every time the fitness is measured.
...
principles-based way to compute the fitness and end the algorithm. Besides, any approach you choose, it is usually impossible to reach a global optimum short 
% Antonio - short???
% short of - a menos que - JJ
of managing to obtain a bot that, as in Sun Tzu's ``Art of war'' \cite{tzu2013art}, is able to beat the opponent without
...
However, using only one or a small number of opponents may lead to generate overfitted solutions that only beat the  enemies that have been used during the 'training' phase (to compute the fitness) \cite{DBLP:journals/jcst/MoraFGGF12}.
 

However, these two issues are usually not taken into accout and the usual approach is
using a fixed number of evaluations or amount of time, usually given
by the game or challenge conditions. When this rule is not imposed by
a challenge or some other condition, this is not the right
approach for finding the best autonomous agent, or even one that is
...
These three problems, two of them (noise and unkown optimum)
...
termination conditions and  comparing them with others and among
themselves when trying to
...
quality of that solution as metrics. 
