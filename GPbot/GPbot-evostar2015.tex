...
The \textit{FI} criterion is useful to `detect' local optima. Increasing the restriction value of this method makes the EA to use more generations to escape from a local optimum, obtaining significantly better results. As the EA can quickly converge to a local optima, this method is equivalent to a fixed (but unknown) number of generations enough to make the population stagnate. However, the results show that this criterion has stopped in local optima that other methods have surpassed.
% Antonio - @Antares, @ Fergunet - esto no lo entiendo bien. �El m�todo es equivalente al n�mero de generaciones para hacer que la poblaci�n se estanque? �Eso es bueno?�Se 'hace' que la poblaci�n se estanque o se detecta que eso pasa?
