...
it as much as you have got, which is fair if you are designing a bot %maribel, el got no se suele poner 
...
it needs to be done in conditions that are as similar as possible, which %maribel sustituir "conditions that are as similar as possible" por "similar environments, "
might make {\em as much as you have got} a bit short on
details. Methods that are going to be compared need to have an %Maribel usar are going es si lo fueras ha hacer ahora mismo, es presente continuo, yo no usaría ni ese tiempo ni por supuesto el futuro, a no ser que estés hablando de las líneas futuras del artículo. Yo pondría "Compared methods need stop by the same independent stopping criteriun, since... 
...
victories are obtained. %Maribel, achieved en vez de obtained. 
...
solutions (the best, if possible). %Maribel: Anadir al final de la frase, saving resources and time. 
To measure the quality of each 
method, we will %maribel: quitar el will
...
known and/or is noisy. Next in Section \ref{sec:met} we will %Maribel: quitar el will de la frase
