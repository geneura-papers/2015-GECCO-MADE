...
%
\author{A. Fern\'andez-Ares \and P. Garc\'ia-S\'anchez\inst{1} \and A.~M. Mora\inst{1} \and  P.~A.~ Castillo\inst{1} \and J.~J. Merelo\inst{1} \and M.~G. Arenas\inst{1} \and G. Romero\inst{1}}
%
...
The first issue is that optimal bots are either unknown (and thus unusable as termination condition) or unreachable. In most EAs trying to find optimal bots fitness is evaluated through game playing. Many times it is found to be 
...
Thus the main objective of this paper is to test several termination conditions 
in order to find the one that yields optimal solutions within a restricted 
...
war'' %\cite{tzu2013art} A muchos revisores no le ha gustado la broma, y nos colamos de espacio
...
%those found in the state of the art.  %A. Tengo dos revisores que no entienden esta frase. Y yo tampoco :(
...
obtained good results in previous works \cite{EvoStar2014:GPBot}.% Este no
...
% The rest of the paper is organized as follows: Section  establishes the 
...
% presents the different termination conditions %maribel Borrar desde aquí hasta "and how ... " osea que la frase quedaría mucho más corta y sencilla "Next in Section \ref{sec:met} we present the different termination conditions and how they contribute to ...
...
% % Antonio - esto habr�a que reescribirlo mejor y con los contenidos
% % reales. ;D
% % Se ha hecho?
...
\begin{itemize}
    ...
    \item \texttt{[AO] Age of Outliers}: if the age of individual is an outlier in the comparison with the rest of the population then it would be potentially an optimal solution and the algorithm can be stopped. % why?
...
% Antonio - @Antares - SCORE en min?scula, plis ya. :D
...
% Antonio - @Antares - como ya te dije, yo no soy partidario de usar escalas logar?micas en los boxplots, ya que se 'falsea' la informaci? que estos ofrecen (el ancho del boxplot es importante). Mira por ejemplo el RT_n/4 o RT_n/8, parecen mucho m? dispersos que RT_n/16, pero ?realmente lo son?
...
% Antonio - Antares - mira si est?bien explicado esto
...
% - Las leyendas deber?n poner 'Percentage of victories' o '% Victories', y 'SCORE per individual' (en may?cula o en min?scula, pero siempre igual). ;D
% - ?Cu?tos son 'several opponents'? Hay que poner cu?tos rivales se han considerado y cu?es en el texto. 
...
\captionof{table}[foo]{Average results of every criterion for the three measures: Number of generations (G), Score (S), and  Victories in benchmark (V); plus the Completion rate in experiments (R). Relative values are computed with respect to \textit{NG\_30.0}.}
\label{fig:table}
\end{minipage}
\end{figure}
...
% El AO no es un buen criterio de parada ya que los resultado prueban que aumentar las restricciones no garantizan una mejora de los individuos, ya que no exiten diferencias significativas entre los valores cercanos del criterio.
...
%Este criterio ha resultado muy bueno detectando cuando el algoritmo se ha estancado en un óptimo local. A mayor restricción del criterio, mayor es tiempo que se da algoritmo para salir de un óptimo local, ya que se dan más generaciones para intentarlo. Dado que es posible que un EA converja rápidamente al mejor individuo de la población inicial, este criterio se comporta de forma similar a un criterio de parada basado en número de generaciones suficientes para que este suceso ocurra. En la experimentación, este criterio de parada ha detenido las ejecuciones en óptimos locales sin permitir mejora que otros criterios si han permitido. Sin embargo su capacidad de detección de óptimos locales, puede resultar muy como mecanismo de reinicio.  
...
% Antonio - @Antares, @ Fergunet - esto no lo entiendo bien. ?El m?odo es equivalente al n?mero de generaciones para hacer que la poblaci? se estanque? ?Eso es bueno??Se 'hace' que la poblaci? se estanque o se detecta que eso pasa?
...
%*** Generalizar esto a cualquier problema => hacerlo una metodolog?
%***
% Co?, ?es que no pod?s poner comentarios? -JJ
...
% Antonio - hay much?imas referencias. Se pueden quitar unas cuantas
