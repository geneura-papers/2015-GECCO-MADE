...
% y?  JJ
Then, in most games, fitness is noisy and (usually) re-evaluated every
generation, so that even if you consider reaching a certain level of
fitness as a termination condition, there is no guarantee that the
individual with that fitness will not fall below the threshold next
generation it is evaluated, or, for that matter, that it is actually
optimum.  
...
time and evaluations. Eventually, what we are trying to find is a
termination condition that is able to find a good solution in a
certain amount of time, and that allows researchers to compare between
different evolutionary algorithms as fairly as possible, that is why in this paper 
