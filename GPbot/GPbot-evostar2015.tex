...
%
\begin{abstract}
Evolutionary Algorithms are frequently used as a mechanism for the optimization of autonomous agents in games (bots). % quitado offline que no se entiende en este contexto - JJ
Determining the end of the optimization process in this stage is not as easy as it would a priori seem. 
...
% y?
Then, in most games, fitness is noisy and re-evaluated every generation, so that even if you consider reaching a certain level of fitness as a termination condition, there is no guarantee that the individual with that fitness will not fall below the threshold next generation it is evaluated, or, for that matter, that it is actually optimum. 
...
Usually running for a constant amount of generations is often considered as an acceptable criteria, if the obtained bots are good enough. However this is not necessarily the best approach, since further optimization could lie ahead, so other options must be explored. 
...
possibility of combining several of them as termination condition so
...
time and evaluations. In this paper 
...
them and, eventually, selecting one for future designs. % Nos falta
                                % qué criterio vamos a seguir para
                                % seleccionar un criterio sobre
                                % otro. ¿Simplemente el fitness? ¿Una
                                % integral del fitness sobre las
                                % generaciones? 
We will use a particular game, Planet Wars, which has been studied
extensively in the literature, but the conclusions to our study could, in principle, be extended to any RTS or even any game with non-deterministic match results and with an
