...
% Reescrito todo que el abstract ocupa casi una p�gina
...
% Antonio - Tambi�n se habl� de extrapolar los resultados a otros problemas con caracter�sticas comunes a estos, es decir: Fitness ruidoso, sin �ptimo conocido, etc, no?
% Antares - Ser�a lo suyo . Por ejemplo, hablando con Gustavo y Maribel, 
...
%Maribel: el número de generaciones y el número de evaluaciónes
%están relacionadas sólo en poblaciones de tamaño fijo. Yo las
%trataría como condiciones de parada en algunos ámbitos, aunque no
%sé si nos interesa o no tratarlos separados o simplemente decir que
%en este caso son lo mismo aunque para otros entornos no tiene por
%qué serlo.
% Maribel da exactamente igual. Lo importante es el n�mero de
% evaluaciones. - JJ
...
% �ptimo - JJ
% Antonio - Yo esa cita la quitar�a, aunque le d� un componente
% literario/gafapastiense al art�culo. Tampoco veo por qu� no se puede
% obtener un �ptimo por no conocer al oponente antes de la
% evaluaci�n. Si es un manta, le puedes ganar siempre...
% Af�. Es literario-gafapastiense-bromista. �Qu� tiene de malo? - JJ
% Antares - A mi gusta xD
% Antonio - a mi no me molesta la frase, pero la referencia no hace falta, creo yo. ;D
% �qu� hay de la segunda parte de mi comentario
...
very good results in previous works \cite{EvoStar2014:GPBot:anon}.% Este no
                                % est� en la bibliograf�a. Por favor,
                                % a�adidlo a geneura.bib - JJ  
                                % Antonio - hecho
...
% Vamos a poner un m�todo como base: n�mero fijo de evaluaciones, por
% ejemplo. A partir de ah� yo considerar�a el n�mero de evaluaciones
% adicionales y la mejora adicional al fitness. Tambi�n el fitness
% absoluto, claro. - JJ
% Antonio - yo digo que el fitness (el usado durante la evoluci�n)sea
% un mero indicador de lo buena que es cada soluci�n, pero que eso se
% contraste con una evaluaci�n final de los mejores individuos
...
% para decidir cu�l de ellos es realmente bueno y cu�l es,
% eventualmente o te�ricamente, el mejor.
%Habr� que tener en cuenta tambi�n el tiempo, digo yo. - JJ
% Antares - Son tres m�tricas a ser consideradas: Bondad (fitness), Tiempo (Generaciones), tasa de finalizaci�n (no todos los SC han terminado en todas las ejecuciones). Sobre la bondad, se est� ejecutando una bater�a de test en m�ltiples enfrentamientos :)
...
% Antonio - adem�s de esto, que hay que hacerlo para dejar la frase, hay que demostrar de forma 'impepinable' (m�s o menos) que lo que se propone funciona.
...
% Antares - ¿A qué te refieres con diversidad?
...
%which is better for checking some of the stop criteria to test. % �por qu�? - JJ
%Specifically those population-based ones, which are normally more sensitive to diverse populations.
%maribel: Por qu� es mejor tener diversidad para probar criterios de parada?, esto ser� mejor para algunos criterios, pero no para todos, de hecho los criterios de parada m�s utilizados, como el n�mero de evaluaciones o el n�mero de generaciones, no tienen nada que ver con la diversidad de la poblaci�n. S� es verdad que es importante para criterios basados en alguna metrica basada en la poblaci�n, como el fitness medio en relaci�n al mejor fitness, o criterios basados en alg�n tipo de distancia entre los individuos de la poblaci�n, pero no para todos los criterios de parada en general, como parece que dice la frase anterior, as� que creo que hay que justificar mejor el que el algoritmo sea generacional. Pongo a continuaci�n una posible justificaci�n a ver qu� os parece.
% Antonio - Pos eso es lo que habia puesto yo. XD 
% Me has movido al frase en la que aclaraba eso mismo. Adem�s de que desde el principio pon�a 'some of the stop criteria'. ;D
...
%FERGU: He comentado la intro de arriba y he vuelto a escribirla abajo justificando por qu� usamos el GP generacional

%The first step of the methodology requires to select the most adequate EA to the problem we are addressing. 
% Antonio - �Qu� pesaos con lo de la metodolog�a! Jo�, que aqu� la metodolog�a va de elegir bien los criterios de parada, no de dise�ar el algoritmo. Eso ya lo tendr� hecho el que vaya a usar nuestro m�todo para elegir c�mo/cu�ndo parar. :/
...
% Antonio - yo quitar� esta frase, o pondr�a m�s citas, una de dos. ;) FERGU: a�adida referencia
...
% Antonio - Esto no tiene nada que ver con que se haya elegido un enfoque generacional. Vamos que no veo en qu� ayuda eso... :(
% De hecho, para el ruido ir�a mejor un estacionario, creo yo.
...
%TODO FERGU: comentar y justificar el resto de par�metros del GP FERGU: hecho abajo
...
\begin{itemize}
    \item \texttt{[NG] Number of Generations}: it is the classical termination
      ...
      baseline for comparison with the other approaches. % En ese caso yo creo que no habr�a que probar tantos valores diferentes.
...
% Antonio - @Antares, explica esto, que no me qued� claro lo de la IQR... :D
% Vale, lo pongo primero en espa�ol y luego intento hacerlo en ingl�s. En este criterio de parada hemos empleado el concepto estad�stico de frontera siperior para definir un umbral. Por definici�n, todo valor de la muestra que supere esa frontera est� considerado un punto outsider. Para calcular dicha frontera se necesitan los cuartiles de la muestra Q1 y Q3, con los que se calcula el rango intercuart�lico (IQR). El valor de la frontera superior se calcula como = Q3 + 1.5 + IQR . Sin embargo, el valor 1.5 puede ser ajustado a voluntad se denomina rango de la frontera. En nuestro caso, hemos empleado el valor est�ndar de 1.5 as� como valores menos restrictivos (1.0) y m�s restrictivos (2.0 y 2.5).
...
% Maribel: En un algoritmo generacional cl�sico, se reemplaza toda la poblaci�n con los hijos menos el mejor. Pero nosotros no estamos usando este reemplazo, as� como as�, sino que hacemos uni�n de los hijos y los padres y nos quedamos con los mejores N no? Si esto es as�, ser�a una cosa a comentar en la secci�n de GA features.
% Antonio - efectivamente, he intentado contarlo mejor debajo. En todo caso hay que aclararlo en la descripci�n del algoritmo. - TODO -
% Aqu� hab�a que haber hecho una aclaraci�n o referencia al mismo. Ya lo he aclarado - JJ
...
% Antares - @Antonio eso era antes, la idea es que N es siempre el tama�o de la poblaci�n, y en este caso, n el valor m�xmo de reemplazamiento.
% Antonio - @Antares, cuidao con la variable 'N', que es el n�mero de batallas en la funci�n Score... ahora est� bien.
...
% Antares - �Esto no lo hab�as cambiado t�? ya no son estos :(
% Antonio - el que comenta es @JJ, no yo. :P
%           Ya lo he cambiado, intentando justificar esto, pero weno, es un poco chungo...
% Antares - Ya el problema ha sido que los valores ``chulos'' no arrojasen resultados. De todos modos, es lo que te coment� una distribuci�n uniforme desde la ``mitad'' del fitness hasta el valor m�ximo.
...
% Antares - S� se contempla el ruido, ya que se siguen haciendo re-evaluaciones. 
...
Four possible values will be tested: 3, 7, 10 and 15 generations. %maribel y por qu� estos? est� fijado antes en alg�n otro paper como el del n�mero de gneraciones?
...
% Antonio - aclarado. Es la m�s dif�cil de justificar. 
% JJ - yo probar�a m�s valores. En todo caso, el fitness variar� cada generaci�n si se vuelve a ejecutar, �no? 
% Antonio - Claro, el fitness cambia, pero se considera el fitness mejor global en cada generaci�n, no el del individuo que se considerame el mejor. Lo he aclarado en el texto.
% Antares - si pero el ``fitness'' m�ximo de la poblaci�n alcanzado hasta el momento no cambia, que es con el que se realiza la comparaci�n.
\end{itemize}
...
% Habr� que decir qu� se va a medir y por qu�. �Qu� se compara? �Por qu�?
%
...
The experiments conducted have involved 36 runs (this number has been used in order to profit the computational power of a cluster with this number of nodes), each of them configured including all the defined stop criteria, and also adding an extra termination one, i.e. getting to 500 generations, in order to avoid non-ending runs.
...
\end{center} 
\caption{(Left) Scores of the best individuals (of all runs), grouped by criterion. (Right) Kruskal-Wallis test of samples of each criterion by score (black means no statistical significant difference has been found)}
...
The first detail to notice in the figure is the non-presence of some of the commented stop values for the \textit{Fitness Threshold} criterion, namely 28 and 30, because they have not been reached in any of the runs.
...
As it can also be seen in Figure \ref{fig:boxplotscore} (left) all the criterion are well defined, since all the scores grow in every criterion block, so as more restrictive the criterion is (they are met with a lower probability), the higher the obtained score is. The score function is working well, even with the presence of noise, as the wide of the boxplots (score dispersion) demonstrates when the criteria become more restrictive. This is true in all the cases except in the \textit{Age of Outliers} which is so far the criterion with the worse results, as it is also proved by the statistical test which does not find significant differences between the scores obtained by every age-based criterion and the previous and next ones.
...
With respect to the number of generations reached by every criterion, Figure \ref{fig:boxplotgenerations} (left) shows their distribution in criterion blocks. It is complemented (right) with the results of the Kruskal-Wallis test, like in the score graphs.
...
\end{center} 
\caption{(Left) Generation attained (of all runs), grouped by criterion. (Right) Kruskal-Wallis test of samples of each stop criterion by generations (black means no statistical significant difference has been found).}
...
This figure shows that every stop criteria needs more generation for determine that the algorithm have to stop when the criterion is more restrictive. \textit{AO} stop criterion is the faster, needing less generation but it's the worst in the score. For other hand, the \textit{FT} stop criterion it's the one that need more generations to finish, and it's possible that it never ends (in \ref{fig:table} shows the completion rate in experiments of each stop criteria). The \textit{FI} scale well, needing few turns to determine the stop. Finally the \textit{RT} it's the one that scale more pronounced, needing few turns in the minus restrictive case and a variable number in the more restrictive (and it's possible that never stop, like \textit{FT}).

The Kruskal-Wallis test shows that the more restrictive \textit{FI} stop criteria aren't statistical significant difference with a \textit{Number of Generations Stop Criterion}. It shows too, that there aren't difference between the near values of \textit{AO} criteria. 
...
\end{center} 
\caption{Victories in benchmark versus severals opponents in severals maps (training and not training) and linear regression of the Score (fitness) with the result of the benchmark (Victories), with a $R^2=0.9351$ } %Antares - TODO: Ahora lo completo
\label{fig:victories}
\end{figure}

In addition to the score, a benchmark ($\in[0,1]$) based in battles versus several bots common in the bibliography and maps (training and not training) it's passed to the best population of each criterion for all execution. The result of these benchmark it's displays in Fig \ref{fig:victories}. For be sure of the goodness of the score function it's compare to the benchmark result's getting an excellent result.


Finally the two principal measures are compare in Figure \ref{subfig:comparative} and all the measures are presented in the Table \ref{fig:table}. In this table is presented too the completion rate of the stop criterion, that indicate the number of executions that have finished in the experiments. 
...
\captionof{table}[foo]{Comparative of each criterion with three measures: Number of generations (G), Score (S), Completion rate in experiments (R)  and  Victories versus several bots in several maps (V). (Relative measure by \textit{NG\_30.0}).}
\label{fig:table}
\end{minipage}
\end{figure}
...
% Antonio - Antares - Pregunta general. �por qu� los valores en las gr�ficas llevan un decimal '.0' si son enteros? �No estar�an mejor sin �l? �No quedar�a m�s claro al haber menos texto? :D
% Antares: @Antonio, por seguir una nomenclatura com�n, pero si quieres podr�a intentar cambiarlo, pero me iba a llevar tiempo.
...
% No se usa ning�n mecanismo para ``paliar'' el ruido. S� se intenta trabajar con �l.
% Antares - �Se podr�a comentar tambi�n que alguno de los criterios de parada podr�a ser empleado para alg�n mecanismo diferente del algoritmo, como un
% ``cataclismo'' o una variaci�n de los pesos o algo as�? 

% Antares - A�ado las conclusiones primero en espa�ol:
%El peor criterio de parada encontrado es el m�todo \textit{[AO] Age of Outliers} basado en la edad de los individuos de la poblaci�n, que es el m�todo que consigue peores resultados en el score ofreciendo una convergencia pr�cticamente nula de las soluciones encontradas en la poblaci�n al aumentar lo restrictivo del m�todo. Esto puede ser un indicativo de que la antiguedad de los individuos no es representativo de la bondad de los individuos. Adem�s, el test de Kruskal-Wallis, determin� que exite poca variaci�n entre las muestras de las distintas variaciones del m�todo.
%Maribel, hago una primera aproximaci�n en ingl�s para este p�rrafo. Leerla a ver si es eso exactamente lo que Antares ha querido decir en el pa?rafo en espa�ol, porque yo 
...
%Por otro lado, el mejor criterio encontrado respecto a la bondad de los individuos es el \textit{[FT] Fitness Threshold} basado en umbrales de fitness. Sin embargo, las dificultades para definir un umbral de fitness correcto en un problema de nuestra naturaleza hacen este m�todo no recomendado, aunque si ideal si puede determinarse. Adem�s, es el m�todo que m�s generaciones requiere, con el riesgo de que la ejecuci�n del algoritmo no llegue a terminar, al estancarse la poblaci�n y no ser capaz de convergir al valor frontera.
...
% El m�todo basado en \textit{[FI] Fitness Improvement} es un m�todo bastante r�pido (en cuanto a generaciones), pero que no alcanza resultados muy buenos (en cuanto score). A pesar de que no pueda ser empleado como criterio de parada �ptimo, su uso puede resultar interesante como apoyo al algoritmo como mecanismo de multiarranque del algoritmo o de cataclismo.
...
%Por �ltimo, el m�todo de \textit{[FT] Fitness Threshold} es el que ofrece unos resultados m�s consistentes en todas las m�tricas. Dado que se basa en la tasa de reemplazo de los individuos, en cierta manera est� midiendo el constante crecimiento del fitness dado que los individuos nuevos tienen que tener mayor fitness para entrar en el pull de la nueva poblaci�n, pero sin hacer uso del propio fitness (que sabemos que por su naturaleza ruidosa no es totalmente confiable). 

At least, \textit{FT} provides the best results if we combine both metrics, generations and score. It is based on replacement rate, so it measures, in a certain sense, how the population increases their abilities because the new individuals would have a higher fitness to be included into the population, but we are not testing their fitness themselves.  


%Antares - Blablabla correspondencia lineal entre los resultados de la bater�a de test de bondad y el score con un $R^2=0.9351$
