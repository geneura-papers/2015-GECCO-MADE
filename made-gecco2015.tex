...
According to the work by \cite{ANON}, systematic tests on the parameters of the virtual world show that conflicts are essential for the drama
emergence. For this reason, in this paper we propose a methodology to formally define a conflict and under which circumstances a virtual world can offer it.

%FERGU: hablas de las capas y todavía no las has definido bien, hazlo aquí (y pon la figurica que estabas haciendo).

We propose a layered model that takes into account... are built in the previous ones... whatever
\begin{itemize}
...
%FERGU: This study will lead us no me gusta para introducir el párrafo
This study will lead us to define fitness functions (evaluation layer) that rely in the definition of abstract archetypes (abstract archetypes layer) expressed as logical predicates. These archetypes do not depend on the world implementation, but in base or primitive predicates that the virtual world may implement (implementation layer). %FERGU: no usar don't ni contracciones
...
We will use Predicate Logic to present the abstract archetypes layer and the implementation layer. %FERGU: Y por qué la usamos? This is interesting because PEPICO says that... por ejemplo


Following the definition by \cite{wikipedia} in mathematical logic, predicate logic is the generic term for symbolic formal systems like first-order logic, second-order logic, many-sorted logic, or infinitary logic. This formal system is distinguished from other systems in that its formulae contain variables which can be quantified. %FERGU: AGH, no copies párrafos tal cual, queda feísimo xD. Además, creo que hay artículos científicos del tema que estamos tratando que son mejor cita que la wikipedia (lo que te he dicho arriba). Este párrafo fuera, que no dice nada interesante.

%FERGU: Resumiendo, hay que vender lo de los predicados bien y con referencias chulas

In this paper, the conflict, as element of vital importance for the drama generation, is studied through the monomyth. Joseph Campbell's monomyth, or the hero's journey as described by Vogler in \cite{vogler}, is a basic pattern that its proponents argue is found in many narratives from around the world. This widely distributed pattern was described by Campbell in The Hero with a Thousand Faces \cite{hero}. Vogler defines the following archetypes:
%TODO summaries from http://www.thewritersjourney.com/hero%27s_journey.htm
\begin{enumerate}
\item Heroes: Central figures in stories.  Everyone is the hero of his or her own myth.
\item Shadows: Villains and enemies, perhaps the enemy within.  The dark side of the Force, the repressed possibilities of the hero, his or her potential for evil.  Can be other kinds of repression, such as repressed grief, anger, frustration or creativity that is dangerous if it does not have an outlet. %FERGU: no usar "doesn't". Todas estas movidas de las cosas oscuras sirven para el paper?
...
\item Shapeshifters: In stories, creatures like vampires or werewolves who change shape.  In life, the shapeshifter represents change.  The way other people (or our perceptions of them) keep changing.  The opposite sex, the way people can be two-faced.
...
%FERGU: enlaza la lista anterior con lo siguiente: Ejemplo: This amount of archetypes need to be represented as PATATIN as possible POR TAL y TAL, therefore we propose...

We propose the following base predicates that the implementation layer may provide:
\begin{enumerate} 
...
\item $ShortPeriod(t_1,t_2)$: $true$ if the difference between the moments $t_1$ and $t_2$ is short (this value will depend on the world passage of time).
...
%TODO Agnadir una imagen que ilustre lo de las 3 capas
%TODO continuar con las capas 2 y 3


Once the archetypes are provided, three types of fitness functions are provided. The first one will consist on %TODO FERGU: qué quiere decir "provided"? quien las provee? Por qué? Prefiero quitar este párrafo y explicar las funciones fitness en general, cómo se define una función fitness, como saber si una función fitness es de calidad y luego en el experimental setup describirlas (vamos, las estaba describiendo yo ahora abajo xD)
