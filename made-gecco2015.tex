...
and different productions where PGC is applied are winning international awards for the first time, proving that PGC can also be an added value due to the re-playability and richness of the result, attributes very appreciated by the videogames players.
...
According to the classification by Hendrikx et Al. \cite{Hendrikx:2013:PCG:2422956.2422957}, PGC can be applied to many different areas in videogames, such as derived content, design, scenarios, systems, space and game units. Our paper is focused in the creation of scenarios, and more specifically in the backstories of the non-player characters in massive worlds, used to provide realism, create an interesting atmosphere and promote the appearance of secondary goals.
In this context, and following the work by Garc\'ia-Ortega et Al. \cite{garcia14my}, creating backstories using hybrid 
\textit{Evolutionary Computation - Agent-Based Model} approach 
(for further references EC-ABM) have been proved to work in virtual words where agents with particular personalities or \textit{profiles} live in a \textit{map} and interact in a \textit{space-time} dimension, generating high level behaviors or \textit{archetypes} that can emerge.
These archetypes are selected by the creator of the videogame depending
of the setting needs. The authors demonstrate in their work that sensible non-evolutionary parameters as the number of agents, the world size,
the execution time or the source of conflicts affect drastically the quality of the solution given by the EC-ABM approach and that the 
appearance of conflicts promotes the appearance of archetypes, \textit{ergo}, better solutions.

In our paper, we focus on these conflict-based archetypes and we classify them based on their relation to the source of conflict, allowing the creator of a story predict their appearance when certain parameters of the world are established. Also, different conflict-based patterns extracted from classical literature are proposed, explained and designed.
In order to implement these archetypes, the MADE open source simulator is used. MADE works as follows: every individual in the evolutionary algorithm is described by a chromosome that represents the parameters of one (or several kinds of) finite state machine (FSM) \cite{FSM_Booth} that is set to \textit{live} (interact) in a simulated environment.

In the other hand, promoting different archetypes relies in an appropriate fitness function for a virtual world. In this paper we propose and test different approaches based in the archetypes of the agents that inhabit the world, their speed of apparition, their distribution, etc, selecting those that generate better solutions in the experiments performed.
...
of conflict-based archetypes. The next section \ref{sec:met} presents the methodology,followed by the experimental setup in Section \ref{sec:exp}, whose results are shown in Section \ref{sec:res}. Finally, we present our conclusions to finish the paper.
