...
emergence. For this reason, in this paper we propose a methodology to formally define a conflict and under witch circumstances a virtual world can offer it.
This study will lead us to define fitness functions (evaluation layer) that rely in the definition of abstract archetypes (abstract archetypes layer) expressed as logical predicates. These archetypes don't depend on the world implementation, but in base or primitive predicates that the virtual world may implement (implementation layer).
...
\item Shadows: Villains and enemies, perhaps the enemy within.  The dark side of the Force, the repressed possibilities of the hero, his or her potential for evil.  Can be other kinds of repression, such as repressed grief, anger, frustration or creativity that is dangerous if it doesn?t have an outlet.
...
\item Threshold guardians: The forces that stand in the way at important turning points, including jealous enemies, professional gatekeepers, or your own fears and doubts.
...
\item $Trickier(i_1, i_2 ,t)$: $true$ if the element $i_1$ if considered as ``trikier'' or ``funnier'' than the element $i_2$ in the moment $t$.
\end{enumerate}
...
Once the archetypes are provided, three types of fitness functions are provided. The first one will consist on %TODO
...
\subsection{Environment, algorithm and parameter configuration}
...
In this paper we will use the MADE world's parameter configuration that obtained the best results in terms of archetype generation in the study presented in \cite{garcia14evostar}. The best combination of parameters was the smallest map size (5x5), the bigger number of virtual days (256), the fewer rate of food (1/8 of the number of cells), the lower number of profiles (1) and the bigger population size of the GA (256). The same GA used in that paper will be used to test the different fitness functions. The parameters to use are shown in Table \ref{tab:parameters}.
