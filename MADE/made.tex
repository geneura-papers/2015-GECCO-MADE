...
vast search space for the evolutionary algorithm
 % TODO: <RH> Y no solo evolutionary, pero se si liar más el abstract</RH>
parameters that it
...
%TODO revisar toda la sección, de momento estoy poniendo la info más o menos redactada pero falta la coherencia


It's important to mark out the nature of the problem
according to the taxonomy described by \cite{togelius2011search}.

\begin{description}
\item[PCG] The Massive generation of backstories using the methodology described in
\cite{ALIFEV} lies in the creation of a whole world with its
characters, relations and plots, so it can be classified as a
it{procedural content generator} (for further references PGC).
\item[Optional] The content itself (backstories) is \textit{optional} for the main
plot but it contributes to the realism of the user experience.
\item[Stochastic] The generation has to be \textit{stochastic} for each execution,
enabling the reuse and replayability of the game.
\item[Offline] Every optimization has to be performed during the game 
development (\textit{offline}).
\item[Search based] The system is optimized with a
generate-and-test algorithm.
\end{description}

In this context different approaches has been carried out,
following the survey by \cite{arinbjarnar2009critical}:
...
was used to create
infinite soap opera style stories driven by goals provided by the author.
It used fragments of stories to assign roles to stereotypical characters
and so relying on the reader the assumption of the characters' motivations.
...
for the story and for each character.
Afterwards, in 2008, more recent investigations \cite{riedl2008toward} used the same idea,
conceptualizing a vignette as a small story assumed as ``interesting'' and
a story as a sequence of vignettes.

Other, researches were based in emerging plots from agents' interactions:
In 2007, \cite{swartjes2007emergent} Virtual storyteller, used agents that improvise using
techniques from improvisational theatre, a plot guide and a narrator.
In 2011, \cite{nairat2011character} proposed a generative drama approach
that integrates human creativity by using an agent-based system where
the characters are developed using interactive evolution.



%TODO Coherencia: mediante MAS




%TODO Optimización de MAS mediante GAS (articulo Evolutionary computation and agent-based modeling: biologically-inspired approaches for understanding complex social systems)

%Mencionar a \cite{cioffi2012evolutionary}.

%``a combined EC-ABM approach illus-
%trated through the RebeLand model of a simple but complete polity system. Results
%highlight tax rates and frequency of public issue that stress society as significant fea-
%tures in phase transitions between stable and unstable governance regimes. These ini-
%tial results suggest further applications of EC to ABM in terms of multi-population
%models with heterogeneous agents, multi-objective optimization, dynamic environ-
%ments, and evolving executable objects for modeling social change.''
%
%``This paper demonstrated a combined EC-ABM approach to the challenge of un-
%derstanding complex adaptive systems in social science. We illustrated our frame-
%work through the RebeLand agent-based model of a simple but rather complete polity
%system. Our results highlighted several variables as being more important than oth-
%ers, such as tax rates and frequency of public issue that stress society. These variables
%represent significant features in phase transitions between stable and unstable gover-
%nance regimes.
%These initial results obtained through evolutionary algorithmic analysis also sug-
%gested further applications of EC to ABM. Multi-population models with hetero-
%geneous agents, multi-objective optimization, dynamic environments, and evolving
%executable objects for modeling social change provide additional EC approaches for
%future research.''
%
%``a combined EC-ABM approach illus-
%trated through the RebeLand model of a simple but complete polity system. Results
...
%TODO Elementos básicos para generación de tramas: Enganchar con Morrell \cite{MorrellFiction06}
...
agent's behaviour. % si lo citas mucho tendrás que anonimizarlo para
                   % el doble blind - JJ
...
%TODO rubén, que no se te olvide hablar de esto en la intro como viene
%en el issue y que esté relacionado con lo que pongo aquí. Por
...
individuals. %TODO citar a un paper de tamaños de poblacion
% Explicar también los parámetros que *no* se prueban y por qué *no*
% se hace. - JJ
...
% ¿podéis NO EXTRAER FRASES MAL ESCRITAS DE PAPERS DE HACE TIEMPO? - JJ
...
of generations (300), % ¿Eso qué significa? - JJ
 the length of the chromosome has an effect in the
 convergence. %FERGU: @raiben habría que hacer lo que dice JJ de
              %estudiar qué eventos se están produciendo. El problema
              %es que habría que lanzar todo otra vez... Pienso que
              %tampoco sería mala idea hacer un estudio respecto al
              %parámetro GENERATION con los Kruskal-Wallis, para ver
              %cuando paran los EAs y justificar mejor este párrafo. 
% No hace falta lanzarlos todos, basta lanzar uno con los mismos
% parámetros y visualizar los eventos - JJ
...
\begin{figure}
        ...
        \caption{Boxplots for parameter D (number of
          days)}\label{fig:boxplotsD} % Ponedlo al revés: for the
                                % number of days (parameter D). Lo
                                % INTERESANTE es el nombre del
                                % parámetro, no el número.
\end{figure}
...
% ¿Por qué diablos decís parámetro D y no lo que hace el parámetro? ¿Y
% por qué usáis with respect en vez de enlazarlo con el anterior?
...
example, a fight) is more difficult. % pero esto habría que probarlo
                                % mirando el número de encuentros, la
                                % densidad de población, y cosas así.
% ¿Cuál es la densidad de población en el mundo 5x5? ¿Cómo evoluciona?
...
count for the fitness, such as % @raiben dilo aquí - JJ
, also need to be low enough so that conflict appear, in this case 1/8
of the world size. % Con un mundo de tamaño 25, ¿aparecen 3 piezas de
                   % comida? - JJ 
