...
\title{World building: analysing the parameters for massive evolutive generation of coherent backstories in fictional environments} 

% Antonio - yo pondría 'evolutionary' en lugar de 'evolutive'. He cambiado 'fiction' a 'fictional' que es adjetivo, creo que está mejor. ;)
...
analyze a set of possible values of the most relevant parameters, obtaining a set of generic rules. 
...
order to generate interesting fictional worlds, including for the
creation of richer environments in modern videogames. 
% Antonio - no creo que la mayoría de autores se centren en idear mundos para videojuegos (por desgracia). :D
In order to make the generation of these fictional worlds and the stories within them truly believable, efficient and massive, several automatic/autonomous methods have been proposed  \cite{garcia14my,nairat2011character}. One of 
...
% Antonio - poner el enlace. Se supone que no es 'nuestro'. Vamos, que no afecta al blind process. ;)
...
% Antonio - hecho. ;D
It follows the methodology presented by García-Ortega et al. in \cite{garcia14my}. They systematized the generation of backstories in massive worlds by means of GAs. Thus, the steps to define the elements of a complex world, and how to optimise their character's behaviour using GAs to extract information, were presented.
...
MADE, 
% Antonio - URL como nota al pie o algo
...
of the proposed method to generate a valid fictional world. 
% Antonio - Pero esta sección también habla de la configuración de los parámetros para los experimentos, ¿no? Si es así, se podría dividir en dos secciones.
% Por cierto, la sección no hace esto que se dice, creo yo. No se explica una secuencia de pasos para sistematizar esto... :_(

Initially, the fitness should be defined considering the desired behaviour for the agents that populate the world, for example, aiming to reach given quantity of some archetypes. Thus, in order to enable the generation of different
personalities, different number of ``profiles'' would be required. This concept,
presented in \cite{garcia14my}, describes the different sets of behavioural
...
Then, the features of this world are also decided taking into account the expected... For example, adding enough days to allow the generation of archetypes. Finally, the interesting plot points are extracted from the log using... % !!! Explain this!!! - JJ TODO @raiben
% Antonio - Pos eso, hay que escribir esto. :D
