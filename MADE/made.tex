...
characters (NPCs) can be considered \textit{Procedural content
  generation (PCG)}, since it implies the generation of oa part of
the game world, namely, the characters and their relations. However,
...
In this paper, we want to extend that study testing the rest of the
...
\item[Profile:] The agents can be parametrised and their behaviour
  ...
  profile (S) is the set of values for these parameters and can be
  optimized by the GA. 
\item[Conflict:] Every action is performed in a place and a time,
  affects to one or more agents and relates to the source of tension.
...
\item[Food:] In this context, the food (F) can be considered as the source of conflicts between the agents; the element that the agents compete for.
\end{description}
...
maximum is the agent's average lifetime. The food is modelled with the parameter (F). Changes in this parameter can increment or decrement
the number of interactions. 

Finally, exploration of the search space can be affected by the GA parameters. In this sense, the most significant
...
the virtual time (D), the food (F) and the population of the GA (P).
...
%of generations (300), % Eso que significa? - JJ FERGU: fuera
