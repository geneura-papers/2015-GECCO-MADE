...
%Rubén - Nota: que rara se me hace la palabra ``idiosyncracy'' :-)
\subsection{Idiosyncrasy of the target problems}
\label{subsec:idio}
...
%TODO Rubén - Hablar de los parámetros que afectan a la calidad de la solución y por qué
As Morrell explains in \cite{morrell2006between}, ``Fiction has three main elements: plotting, character, and place or setting'', so, intuitively, these three elements can affect the backstories:
...
thus different variations of S may derive in different behaviours. The time of the conflict
is affected by the period of time that the world will exist (D).
...
and study how the fitness is affected. The parameters proposed for the typology of problems defined in
subsection \ref{subsec:idio} are number of profiles (S), world size (W),
virtual time (D), leitmotiv (F) and population (P).
...
\subsection{Instantiation of the problem}
...
%TODO Rubén - hablar de todos los puntos que instancia MADE
%TODO 

Initially, the fitness should be defined considering the desired behaviour
for the agents that populate the world, for example, aiming to reach given quantity of some archetypes. Thus, in order to enable the generation of different
...
% Rubén- comento esta sección porque no la veo como parte de la metodología y ya estaba explicado en alife
