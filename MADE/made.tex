...
The agent in the MADE world is a representation of brown rat (formerly \textit{Rattus norvegicus})
a common specie of urban rodent, modelled as a finite automaton that
performs actions like move, eat, attack, defend, escape, find mate or have offspring.
...
other agents. An agent can die of hunger, due to the damage inflicted by other agent or
because of its advanced age.


MADE lets the writer select different archetypes that can be promoted in the world
(hero, shadow, threshold guardian, star-crossed lovers, etc) as described in \cite{ALIFEV}.
...
A \textit{hero} is an agent that has eventually attacked a \textit{villain}.
An \textit{avenger} is an agent that attacks other one in revenge.
...
% Rubén: ¡¡¡ Voy por aquí, subo paper y sigo !!!
