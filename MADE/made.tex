...
personalities % no uses facilitate!!!!! - JJ FERGU: ok, no sabía que estaba mal
...
Therefore, a Kruskal-Wallis multi-comparison test has also been performed, showing significant differences among all parameter values, except for number of profiles (only using 1 profile has significant difference with respect to use 2, 4 and 8), and GA population size (64-128 and 128-256), both values in scenarios Villain and Hero. This can be explained because only one profile is necessary in these scenarios (as the hero and villain archetypes are based in attacking other rats, and therefore, can be shared by the same agent).

All the results have been also plotted to better understand their differences. Figure \ref{fig:boxplotsE1} shows the boxplots for the fitness grouped by parameter for the Villain scenario. As can be seen, the median for D, W and F is 0 (or almost 0) for several values. This can be explained for several reasons. First, a small amount of days (64) is not enough to allow the generation of any archetype, as there is no time to develop a story. This can also be related to the world dimension: larger worlds require more time to be explored, so the chances of an encounter that can trig an event (for example, a fight) is more difficult. Finally, the amount of food, the element that makes most actions appear, also need to be low enough to promote fightings (1/8 of the world size). Similar results are obtained for the scenario 2 (Hero), (see Figure \ref{fig:boxplotsE2}) as this archetype depends on the previous existence of the villain archetype, are based in attack other rats, and can be shared by an agent.

With respect to the Shakespearian scenario %TODO @raiben
...
\begin{figure}
        \centering
        \subfigure[Profiles]{
                \includegraphics[width=3.5cm]{imags/boxplotz1P.eps}
                \label{fig:e1_p}
        }
        \subfigure[Days]{
                \includegraphics[width=3.5cm]{imags/boxplotz1D.eps}
                \label{fig:e1_d}
        }
        \subfigure[World Size]{
                \includegraphics[width=3.5cm]{imags/boxplotz1W.eps}
                \label{fig:e1_w}
        }  
        \subfigure[Food]{
                \includegraphics[width=3.5cm]{imags/boxplotz1F.eps}
                \label{fig:e1_f}
        }
        \subfigure[GA Population Size]{
                \includegraphics[width=3.5cm]{imags/boxplotz1S.eps}
                \label{fig:e1_s}
        }

        \caption{Boxplots for scenario Villain (one archetype needed)}\label{fig:boxplotsE1}
\end{figure}

\begin{figure}
        \centering
        \subfigure[Profiles]{
                \includegraphics[width=3.5cm]{imags/boxplotz2P.eps}
                \label{fig:e2_p}
        }
        \subfigure[Days]{
                \includegraphics[width=3.5cm]{imags/boxplotz2D.eps}
                \label{fig:e2_d}
        }
        \subfigure[World Size]{
                \includegraphics[width=3.5cm]{imags/boxplotz2W.eps}
                \label{fig:e2_w}
        }  
        \subfigure[Food]{
                \includegraphics[width=3.5cm]{imags/boxplotz2F.eps}
                \label{fig:e2_f}
        }
        \subfigure[GA Population Size]{
                \includegraphics[width=3.5cm]{imags/boxplotz2S.eps}
                \label{fig:e2_s}
        }

        \caption{Boxplots for scenario Hero (two archetypes needed)}\label{fig:boxplotsE2}
\end{figure}

\begin{figure}
        \centering
        \subfigure[Profiles]{
                \includegraphics[width=3.5cm]{imags/boxplotz5P.eps}
                \label{fig:e5_p}
        }
        \subfigure[Days]{
                \includegraphics[width=3.5cm]{imags/boxplotz5D.eps}
                \label{fig:e5_d}
        }
        \subfigure[World Size]{
                \includegraphics[width=3.5cm]{imags/boxplotz5W.eps}
                \label{fig:e5_w}
        }  
        \subfigure[Food]{
                \includegraphics[width=3.5cm]{imags/boxplotz5F.eps}
                \label{fig:e5_f}
        }
        \subfigure[GA Population Size]{
                \includegraphics[width=3.5cm]{imags/boxplotz5S.eps}
                \label{fig:e5_s}
        }

        \caption{Boxplots for scenario Shakespearian (five archetypes needed)}\label{fig:boxplotsE5} % Always explain the scenarios, nobody needs to remember which one is which - JJ FERGU: Added label and comment
\end{figure}
